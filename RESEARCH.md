...
## The not found surfaces

- `not-found.js` "is used to render UI when the `notFound` function is thrown within a route segment", and the root `app/not-found.js` additionally "handle[s] any unmatched URLs for your whole application". A route level file renders inside that segment's layout, which is how the portal's header, navigation and tenant accent stay on screen. https://nextjs.org/docs/app/api-reference/file-conventions/not-found - 23 Sep 2026
- The status code is not a free choice. "When streaming, a `200` status code will be returned", and "Because the response headers have already been sent to the client, the status code of the response cannot be updated." Next marks such a page `noindex` so it is not indexed, but the status is still 200. https://nextjs.org/docs/app/api-reference/file-conventions/loading - 23 Sep 2026
- What starts the streaming: "The response body starts streaming when a Suspense fallback renders (for example, a `loading.tsx`) or when a Server Component suspends under a `Suspense` boundary. Place `notFound()` before those boundaries and before any `await` that may suspend." A `loading.js` "will automatically wrap the `page.js` file and any children below in a `<Suspense>` boundary". https://nextjs.org/docs/app/api-reference/file-conventions/loading - 23 Sep 2026
- Proven, not assumed: `/borrowers/[id]` answered **200** for a foreign id because `app/(portal)/borrowers/loading.tsx` wrapped it, while `/scenarios/[id]`, which has no loading file above it, answered 404. Deleting only `borrowers/[id]/loading.tsx` did not change it, because the parent boundary still covers the nested segment. Moving the list page and its loading file into a `(list)` route group scopes the boundary to the list alone, and all four surfaces then answered 404. docs/evidence/not-found-run.txt - 23 Sep 2026

## The architecture diagram

- The diagram is rendered ahead of time by mermaid-cli rather than left for the host to draw. The command is `mmdc`, and the flags used are the documented ones: `-i` input, `-o` output, `-c` a JSON configuration file for mermaid, `-b` background colour, `-w` width. https://github.com/mermaid-js/mermaid-cli - 23 Sep 2026
- `htmlLabels` is set to false. Mermaid otherwise puts label text inside `foreignObject`, and an SVG referenced by an `img` tag is rendered in a restricted mode where that HTML does not paint, which would have shipped a diagram of empty boxes. The rendered file contains no `foreignObject` and the labels are real SVG text. https://github.com/mermaid-js/mermaid-cli - 23 Sep 2026
- Observed directly: the live Mermaid block failed on the host with "Unable to render rich display: Could not find a suitable point for the given distance". The source was rewritten to node to node edges with no edge labels and no `br` tags, and the prose that the edge labels used to carry now sits under the diagram as text. docs/screenshots/readme-architecture-github.png - 23 Sep 2026
